...
export const AdminDashboard: React.FC = () => {
  ...
  const openShareModal = (training: Training) => {
    // FIX: Gunakan window.location.origin untuk mendapatkan domain asli (https://...)
    const origin = window.location.origin;
    const pathname = window.location.pathname.endsWith('/') ? window.location.pathname : `${window.location.pathname}/`;
    
    // Construct Base URL: https://domain.com/#/evaluate/ID
    const shortUrl = `${origin}${pathname}#/evaluate/${training.id}`;

    const minifiedData = {
        i: training.id,
        a: training.accessCode,
        t: training.title,
        s: training.startDate,
        e: training.endDate,
        p: training.processEvaluationDate,
        f: training.facilitators.map(f => ({ i: f.id, n: f.name, s: f.subject, d: f.sessionDate })),
        fq: training.facilitatorQuestions.map(q => ({ i: q.id, l: q.label, t: q.type })),
        pq: training.processQuestions.map(q => ({ i: q.id, l: q.label, t: q.type }))
    };
    
    const jsonStr = JSON.stringify(minifiedData);
    const compressed = LZString.compressToEncodedURIComponent(jsonStr);
    
    // FIX: Gunakan '?data=' agar sesuai dengan RespondentView.tsx
    const fullUrl = `${shortUrl}?data=${compressed}`;

    setShareData({ shortUrl, fullUrl, title: training.title, accessCode: training.accessCode || 'N/A' });
    setCopied(false);
    setShareTab('link');
